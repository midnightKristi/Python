...
0 0 0 20 20 20 40 40 40 60 60 60 80 80 80 100 100 100 120 120 120 140 140 140'''
    ...
    def PPM_load(self, inasciifile):
        ...
        if (self.width <= 0) or (self.height <= 0):
            raise PPM_Exception(
                "The file being loaded does not appear to have valid dimensions ({0} x {1})".format(str(width),
                                                                                                    str(height)))
        ...
        if (self.colormax != 255):
            raise PPM_Exception(
                "Warning: PPM file does not have a maximum intensity value of 255.  Image may not be handled correctly.")
    ...
    def PPM_grayscale(self):
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                gray = (newpixellist[row][col][0] + newpixellist[row][col][1] + newpixellist[row][col][
                    2]) // 3  # adds the rgb values and
                # divides by 3.
    ...
    def PPM_flip_horizontal(self):
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                newpixellist[row][col][0] = newpixellist[-row][col][
                    0]  # sets the the row to flip and become in the back.
